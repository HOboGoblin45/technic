...
class ProgressStore:
    ...
    def update(self, scan_id: str, **kwargs) -> Optional[ScanProgress]:
        ...
        if progress:
            ...
            # Queue WebSocket notification (will be handled by async context if available)
    ...
    async def notify_websockets(self, scan_id: str, progress: ScanProgress):
...
def _main() -> None:  # pragma: no cover
    import uvicorn
    uvicorn.run("api_enhanced_fixed:app", host="0.0.0.0", port=int(os.getenv("PORT", "8000")), reload=True)
