"""
Test suite for enhanced API with progress tracking.
Tests Phase 3D-C implementation.
"""
...
class TestProgressTracking:
    ...
    def test_start_scan_async(self):
        ...
        with patch('api_enhanced.scanner_core') as mock_scanner:
...
class TestWebSocketAndSSE:
    """Test WebSocket and SSE endpoints."""
    ...
    async def test_sse_endpoint(self):
        ...
        scan_id = "test-sse"
        progress = progress_store.create(scan_id)
        progress_store.update(scan_id, status="running", percentage=50.0)
        
        # Start SSE stream in background
        async def complete_scan():
            await asyncio.sleep(0.1)
            progress_store.update(scan_id, status="completed", percentage=100.0)
        
        asyncio.create_task(complete_scan())
        
        # Test SSE stream
        with client as c:
            with c.stream("GET", f"/scan/sse/{scan_id}") as response:
                assert response.status_code == 200
...
class TestPerformance:
    ...
    def test_progress_callback_performance(self):
        ...
        assert elapsed < 0.1, f"Progress callbacks too slow: {elapsed:.3f}s for 1000 calls"
