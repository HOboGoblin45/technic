"""
Enhanced Real-Time Monitoring Dashboard with Historical Data
Streamlit-based UI for ML API monitoring with time-series visualization
Phase 2 Day 3 Task 2: Historical Data Visualization
"""
...
def create_time_series_chart(history_data, title, y_label, color='blue'):
    """Create a time-series line chart from historical data"""
    if not history_data:
        return None
    ...
    fig.add_trace(go.Scatter(
        x=df['datetime'],
        y=df['duration_ms'] if 'duration_ms' in df.columns else df.get('value', []),
        mode='lines+markers',
        name=title,
...
def main():
    ...
    st.markdown("Real-time monitoring with historical trends")
    ...
    if show_trends:
        ...
        if history_data and history_data.get('history'):
            ...
            with col1:
                ...
                # Process history data for response time
                if history:
                ...
                if history:
                    ...
                    if not df.empty:
                        ...
                        fig.add_trace(go.Bar(
                            x=requests_per_minute['minute'],
                            y=requests_per_minute['count'],
                            name='Requests',
                            marker_color='green'
                        ))
                        ...
                        st.info("No request volume data available yet")
                else:
                    st.info("No historical data available yet")
        else:
            st.info(f"No historical data available for the last {time_range} minutes. Data will appear as requests are made.")
