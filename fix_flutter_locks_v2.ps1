# Flutter File Lock Fix Script v2
...
try {
    $null = taskkill /F /IM dart.exe /T 2>&1
    $null = taskkill /F /IM flutter.exe /T 2>&1
    $null = taskkill /F /IM chrome.exe /T 2>&1
...
if (Test-Path $projectPath) {
    ...
    if (Test-Path "build") {
        try {
            ...
            Write-Host "Could not delete build/" -ForegroundColor Yellow
        }
    }
    ...
    if (Test-Path ".dart_tool") {
        try {
            ...
            Write-Host "Could not delete .dart_tool/" -ForegroundColor Yellow
        }
    }
    ...
    if (Test-Path "windows\flutter\ephemeral") {
        try {
            ...
            Write-Host "Could not delete ephemeral/" -ForegroundColor Yellow
        }
    }
}
else {
...
try {
    ...
    Write-Host "Could not stop Windows Search" -ForegroundColor Yellow
}
Write-Host ""
...
try {
    ...
    Write-Host "Could not add exclusion" -ForegroundColor Yellow
}
Write-Host ""
