...
"""
Offline training script for cross-sectional XGB alpha model.
Builds a dataset from cached price history and trains an XGBoost regressor
...
MODEL_LATEST = Path("models/alpha/xgb_v1.pkl")
FEATURES_PATH = Path("models/alpha/xgb_v1_features.json")
...
def train_and_save(
    ...
    versioned_path = model_dir / f"xgb_v1_{version}.pkl"
    model.save(str(versioned_path))
    if save_latest:
        model.save(str(MODEL_LATEST))
    try:
        FEATURES_PATH.parent.mkdir(parents=True, exist_ok=True)
        FEATURES_PATH.write_text(pd.Series(feature_cols).to_json(orient="values"), encoding="utf-8")
    except Exception:
        pass
    model_registry.register_model(
        model_name="alpha_xgb_v1",
        version=version,
        metrics=metrics,
    ...
    print(f"[TRAIN] Saved XGB model to {versioned_path} (latest copy: {MODEL_LATEST})")
...
def main():
    ...
    print("[TRAIN] Building dataset for XGB alpha...")
